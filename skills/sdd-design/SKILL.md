...
Use `<PLUGIN_ROOT>/templates/design.md`（可被 `.workflow/templates/design.md` 项目级覆盖）（10 章完整模板）作为**唯一模板**，按其中章节结构输出到 `.workflow/designs/<NNN>-<中文名>.md`——**必须与对应 spec 同名**（同 NNN + 同中文名，靠 designs/ 目录区分），如 spec `001-用户登录.md` → design `001-用户登录.md`。

> 本 skill 不再内嵌简版模板——`<PLUGIN_ROOT>/templates/design.md`（可被 `.workflow/templates/design.md` 项目级覆盖） 是唯一事实来源，避免模板漂移。模板章节如有不适用项，显式声明"不适用"，不得留空。
...
### 图的生成规范（强制）——唯一标准：`<PLUGIN_ROOT>/templates/design.md`（可被 `.workflow/templates/design.md` 项目级覆盖）「图的生成规范」

> 生成任何图前，先读 `<PLUGIN_ROOT>/templates/design.md`（可被 `.workflow/templates/design.md` 项目级覆盖） 的「图的生成规范」，本节为执行摘要，冲突时以模板为准。
